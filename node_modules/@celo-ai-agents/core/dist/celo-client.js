...
const CELO_CHAINS = {
    alfajores: chains_1.celoAlfajores,
    mainnet: chains_1.celo
};
class CeloClient {
    constructor(privateKey, network, rpcUrl) {
        ...
        this.publicClient = (0, viem_1.createPublicClient)({
            chain: this.chain,
            transport: (0, viem_1.http)(this.network.rpcUrl)
        });
    ...
    getNetworkConfigInternal(network) {
    ...
    async getTransactionHistory(address, fromBlock, toBlock) {
    ...
    async getTokenTransfers(token, address, fromBlock) {
        const currentBlock = await this.publicClient.getBlockNumber();
        const startBlock = fromBlock || currentBlock - 1000n;
        ...
        return transferEvents
            .filter(event => event.topics[1]?.toLowerCase() === address.toLowerCase() ||
    ...
    async registerAgent(agentType, dailyLimit, perTxLimit) {
        const result = await this.writeContract(this.network.contracts.agentRegistry, [
            {
                name: "registerAgent",
                type: "function",
                stateMutability: "nonpayable",
        ...
        ], "registerAgent", [agentType, await this.walletClient.getAddresses().then(addrs => addrs[0]), dailyLimit, perTxLimit]);
        const receipt = await this.waitForTransaction(result);
        const event = receipt.logs.find(log => log.topics[0] === "0x" + "AgentRegistered".padEnd(64, "0"));
        if (event) {
            return BigInt(event.topics[1]);
        }
        throw new Error("Failed to get agent ID from registration");
    }
    async checkAgentPermission(agentId, operation, amount, recipient) {
        return await this.readContract(this.network.contracts.agentRegistry, [
            {
        ...
        ], "isOperationAllowed", [agentId, operation, amount, recipient]);
    }
    async recordAgentAction(agentId, actionType, amount, data) {
        return await this.writeContract(this.network.contracts.agentRegistry, [
            {
        ...
        ], "recordAgentAction", [agentId, actionType, amount, data]);
    }
    async depositToTreasury(token, amount) {
        if (token === this.network.tokens.CELO) {
            return await this.walletClient.sendTransaction({
            ...
            await this.writeContract(token, [
                {
                    name: "approve",
                    type: "function",
                    stateMutability: "nonpayable",
            ...
            ], "approve", [this.network.contracts.agentTreasury, amount]);
            return await this.writeContract(this.network.contracts.agentTreasury, [
                {
                    name: "deposit",
                    type: "function",
                    stateMutability: "nonpayable",
    ...
    async agentWithdrawFromTreasury(agentId, token, recipient, amount) {
        return await this.writeContract(this.network.contracts.agentTreasury, [
            {
                name: "agentWithdraw",
                type: "function",
        ...
        ], "agentWithdraw", [agentId, token, recipient, amount]);
    }
    async batchTransferFromTreasury(agentId, token, recipients, amounts) {
        return await this.writeContract(this.network.contracts.agentTreasury, [
            {
                name: "batchTransfer",
                type: "function",
        ...
        ], "batchTransfer", [agentId, token, recipients, amounts]);
    }
